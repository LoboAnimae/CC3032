...
export function extractTableComponent<T extends CompositionComponent>(inComponent?: CompositionComponent | null) {
  if (!inComponent) return null;
  return inComponent.getComponent<TableComponent<T>>({ componentType: TableComponent.Type });
}

export function replaceTableComponent<T extends CompositionComponent>(inComponent?: CompositionComponent | null) {
  if (!inComponent) return null;
  return inComponent.replaceComponent(inComponent);
}

export class TableComponent<T extends CompositionComponent> extends CompositionComponent {
  static Name = 'Table';
  static Type = 'Table';
